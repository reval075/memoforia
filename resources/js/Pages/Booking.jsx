                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {templatesList.map(template => {
                                            ...
                                            return (
                                                <button key={template.id} type="button" onClick={() => updateForm('selected_template_id', template.id)}
                                                    className={`group p-5 rounded-2xl border-2 text-left transition-all bg-white relative overflow-hidden ${form.selected_template_id == template.id ? 'border-primary bg-primary-50/30' : 'border-beige hover:border-primary-200'}`}>
                                                    {imageUrl ? (
                                                        <img src={imageUrl} alt={template.name} className="w-full h-36 object-cover rounded-xl mb-3" />
                                                    ) : (
                                                        <div className="w-full h-36 rounded-xl bg-beige/50 mb-3 flex items-center justify-center text-xs text-warm-grey">No image available</div>
                                                    )}
